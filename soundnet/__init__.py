from keras.layers import BatchNormalization, Activation, Conv1D, MaxPooling1D, \
    ZeroPadding1D, InputLayer
...
class SoundNet:
    ...
    categories_imagenet = ['tench, Tinca tinca',
                           ...
                           's darning needle, sewing needle, snake feeder, snake doctor, mosquito hawk, '
                           'skeeter hawk",',
                           'damselfly',
                           'admiral',
                           ...
                           'rapeseed',
                           'daisy',
                           's slipper, yellow lady-slipper, Cypripedium calceolus, Cypripedium '
                           'parviflorum",',
                           'corn',
                           'acorn',
    ...
    def __init__(self, model_path=None, verbose=False):
        ...
        self.running = False
        self.model_path = model_path or join(dirname(__file__),
                                             'models/sound8.npy')
        self.model = self.build_model(self.model_path)
        if self.verbose:
            self.model_summary()

    def on_prediction(self, prediction):
        places = self.predictions_to_places(prediction)
        imagenet = self.predictions_to_imagenet(prediction)
        logger.info("PLACES:", places)
        logger.info("IMAGENET:", imagenet)
    ...
    def load_audio(audio_file):
        audio, sr = librosa.load(audio_file, dtype='float32',
                                 sr=SoundNet.sample_rate, mono=True)
    ...
    def predictions_to_places(self, prediction):
        scenes = []
        for p in range(prediction.shape[1]):
            scenes.append(self.categories_places[np.argmax(prediction[0, p, :])])
        # NOTE i have no idea if this is correct, just hacking around someone elses codebase
        # i simply counted predictions and ensured everything sums up to 1
        # i dont understand why we get a list with many repeated labels, should this have a softmax layer at the end?
        scores = [(p, scenes.count(p) / len(scenes)) for p in set(scenes)]
        return sorted(scores, key=lambda tup: tup[1], reverse=True)

    def predictions_to_imagenet(self, prediction):
        scenes = []
        for p in range(prediction.shape[1]):
            scenes.append(
                self.categories_imagenet[np.argmax(prediction[0, p, :])])
        # NOTE i have no idea if this is correct, just hacking around someone elses codebase
        # i simply counted predictions and ensured everything sums up to 1
        # i dont understand why we get a list with many repeated labels, should this have a softmax layer at the end?
        scores = [(p, scenes.count(p) / len(scenes)) for p in set(scenes)]
    ...
    def predict_from_mic(self, seconds=6, record_path='rec.wav'):
        ...
        audio = None
        for i in range(0, int(self.sample_rate / chunk_size * seconds)):
            if audio is None:
        ...
        # TODO make param optional and check for it before saving
        ...
        # TODO figure out correct audio format
        # audio = np.frombuffer(audio)
        # audio = self.preprocess(audio)
        # return self.model.predict(audio)

        # HACK temporary workaround
        return self.predict_from_audio_file(record_path)

    def run(self, seconds=3, sample_rate=16000, chunk_size=1024):
        ...
        while self.running:
            audio = None
            # TODO rolling buffer instead of resetting buffer
            for i in range(0, int(self.sample_rate / chunk_size * seconds)):
                if audio is None:
                    audio = stream.read(chunk_size)
                else:
            ...
            # TODO figure out correct audio format, avoid saving file
            record_path = join(tempfile.gettempdir(),
                               str(time.time()) + ".wav")
            wf = wave.open(record_path, 'wb')
            wf.setnchannels(CHANNELS)
            wf.setsampwidth(sw)
            ...
            p = self.predict_from_audio_file(record_path)
